...
export const fetchVibeEchoesProd = async (): Promise<any[]> => {
  try {
    ...
    if (data && data.length > 0) {
      try {
        const userIds = data.map(vibe => vibe.user_id).filter(Boolean);
        if (userIds.length > 0) {
          ...
          const enrichedData = data.map(vibe => ({
            ...vibe,
            profiles: profilesData?.find(profile => profile.user_id === vibe.user_id) || null
          }));
